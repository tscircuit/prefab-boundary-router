...
test("routes the seven-net Clad1 fanout handoff through its paired vias", () => {
  ...
  const breakoutPorts = breakoutPortDefinitions.map(([id, netId, x, y]) => ({
    portId: `fanout:${id}`,
    netId,
    x,
    y,
  }))
  const viaPorts = viaPairs.flatMap(([first, second]) => [
  ...
  // Expected behavior: all seven nets route through the 40 available via
  // pairs. Current behavior fails before completing the final net.
